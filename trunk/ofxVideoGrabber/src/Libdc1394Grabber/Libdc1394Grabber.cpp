...
void Libdc1394Grabber::setDeviceGUID(uint64_t _deviceGUID){
	...
	ofLog(OF_LOG_NOTICE, "Trying to use Camera with GUID %u",cameraGUID);
	bChooseDevice = true;
}
...
void Libdc1394Grabber::listDevices(){
	...
	for (uint32_t index = 0; index < list->num; index++) {
		...
		ofLog(OF_LOG_NOTICE, "Device[%d].unit = %x, GUID = %u", index, list->ids[index].unit, list->ids[index].guid);
	}
...
bool Libdc1394Grabber::init( int _width, int _height, int _format, int _targetFormat, int _frameRate, uint64_t _deviceGUID )
{
	...
	bool result = initCam( desiredVideoMode, desiredFrameRate, _deviceGUID );
...
bool Libdc1394Grabber::initCam( dc1394video_mode_t _videoMode, dc1394framerate_t _frameRate, uint64_t _deviceGUID )
{
		...
		if(cameraGUID != -1){
			...
			for (uint32_t index = 0; index < list->num; index++) {
				if (list->ids[index].guid == cameraGUID) {
					...
					ofLog(OF_LOG_NOTICE, "Found Camera with GUID %u, it has Index %u",cameraGUID, cameraIndex);
				}
			}

			if (!bFound) {
				ofLog(OF_LOG_NOTICE,"initCam warning: Device Index for unit %u not found, using first device\n", cameraGUID);
			...
			for (uint32_t index = 0; index < list->num; index++) {
				if (list->ids[index].unit == cameraIndex) {
					...
					ofLog(OF_LOG_NOTICE, "Found Camera with Index %u, it has GUID %u", cameraIndex, cameraGUID);
    ...
    if (!camera) {
        dc1394_log_error("Failed to initialize camera with guid %u", cameraGUID);
        return false;
    }
