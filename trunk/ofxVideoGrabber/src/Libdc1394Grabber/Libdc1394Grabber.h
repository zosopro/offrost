...
class Libdc1394Grabber : public ofxVideoGrabberSDK, public ofxThread
{
	public:
		...
		void listDevices();
		bool init( int _width = 320, int _height = 240, int _format = VID_FORMAT_GREYSCALE, int _targetFormat = VID_FORMAT_BGR , int _frameRate = 30, uint64_t _deviceGUID = -1);
		bool initCam( dc1394video_mode_t _videoMode, dc1394framerate_t _frameRateMode, uint64_t _deviceGUID = -1);
		void close();
