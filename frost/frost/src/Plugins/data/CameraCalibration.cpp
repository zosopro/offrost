...
CameraCalibration::CameraCalibration(){
	...
	for(int i=0;i<10;i++){
		objects.push_back(new CameraCalibrationObject);
...
void CameraCalibration::drawGrid(string text, float aspect, int resolution, bool drawBorder, float alpha, float fontSize){
	ofEnableAlphaBlending();
...
CameraCalibrationObject * CameraCalibration::getFloor(){
	return objects[0];
}
CameraCalibrationObject * CameraCalibration::getColumn(int n){
	return objects[1+n];	
}
CameraCalibrationObject * CameraCalibration::getCurtain(int n){
	return objects[4+n];
}

ofxPoint2f CameraCalibration::getColumnCoordinate(int column){
...
void CameraCalibration::applyFloorProjection(float _w, float _h){
	glPushMatrix();
...
void CameraCalibration::applyColumnProjection(int column, float _w, float _h){
	glPushMatrix();
...
void CameraCalibration::applyCurtainProjection(int column, int row, float _w, float _h){
	int n = row + column*3;
	glPushMatrix();
...
void CameraCalibration::applyProjection(CameraCalibrationObject * obj,float _w, float _h){
	glPushMatrix();
